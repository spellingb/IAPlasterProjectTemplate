...
#cloned from https://github.com/AtlassianPS/JiraPS/blob/master/Tools/BuildTools.psm1
...
function Set-GitUser {
    ...
    if (-not (git config user.email)) { git config user.email "ArmorEnterpriseTeam@raxglobal.onmicrosoft.com" }
    if (-not (git config user.name)) { git config user.name "GTS Armor - Enterprise" }
